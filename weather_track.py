...
import pandas as pd   
from weather_utils import (add_observation, get_all_data, search_by_city, search_by_date, get_statistics, get_live_weather, get_weather_forecast)
...
menu = st.sidebar.selectbox("choose a page", ["Home", "Add Observation", "Live Weather", "View Observations", "Search", "Statistics"])
...
if menu == "Home":
    ...
    if st.button("Save Observation"):
        ...
        if city == "":
            ...
            add_observation(date = str(date), city = city, temperature = temperature, 
                            humidity = humidity, wind_speed = wind_speed, condition = condition)
            st.success("Observation Saved Successfully")

elif menu == "Live Weather":
    st.header("🌍 Live Weather")

    city_api = st.text_input("Enter a city")
    save_live_weather =st.checkbox("Save this weather to the CSV file")

    if st.button("Get Live Weather"):
        city_api = city_api.strip().title()
        ...
        if city_api =="":
            ...
            live_weather = get_live_weather(city_api, api_key)
            forecast_data = get_weather_forecast(city_api, api_key)

            if live_weather is None:
                st.error("City not found or the API request failed")
            else:
                st.subheader(f"current Weather in {live_weather['City']}")

                st.write("Temperature:", live_weather["Temperature"], "°C")
                st.write("Humidity:", live_weather["Humidity"], "%")
                st.write("Wind Speed:", live_weather["Wind Speed"], "km/h")
                st.write("Condition:", live_weather["Condition"])

                if save_live_weather:
                    today = str(pd.Timestamp.today().date())

                    add_observation(today, 
                                    live_weather["City"],
                                    live_weather["Temperature"],
                                    live_weather["Humidity"],
                                    live_weather["Wind Speed"],
                                    live_weather["Condition"])
                    st.success("Live weather saved to Data.csv")
